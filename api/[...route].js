...
function sql(){
  if (!sqlClient) {
    ...
    if (!url) throw Object.assign(new Error('server_not_configured'), {status:500});
    sqlClient = neon(url);
  }
  return sqlClient;
}
...
module.exports = async (req,res)=>{
  try{
    ...
    if(route==='auth/username'&&req.method==='POST'){
      const username=String(req.body?.username||'').trim();
      if(!/^[a-zA-Z0-9_-]{3,24}$/.test(username))return send(res,400,{error:'auth/invalid-username'});
      const key=normName(username);
      const row=await getValue('usernames/'+key);
      if(!row?.email)return send(res,404,{error:'USERNAME_NOT_FOUND'});
      return send(res,200,{email:row.email});
    }
